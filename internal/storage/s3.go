...
func NewS3Client(endpoint, region, bucket, accessKey, secretKey string, usePathStyle bool) *S3Client {
	return &S3Client{
...
func (c *S3Client) Test(ctx context.Context) error {
	...
	req.Header.Set("x-amz-content-sha256", sha256Hex([]byte{}))
	c.signV4(req, "", "s3", sha256Hex([]byte{}))
	resp, err := c.httpClient.Do(req)
	if err != nil {
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		body, _ := ioutil.ReadAll(resp.Body)
		return fmt.Errorf("s3 test: http %d: %s", resp.StatusCode, string(body))
	}
	return nil
}
...
func NewS3FromConfig(cfg *admiral.StorageConfig) (*S3Client, error) {
	...
	return NewS3Client(cfg.Endpoint, cfg.Region, cfg.Bucket, accessKey, secretKey, cfg.ForcePathStyle), nil
}
